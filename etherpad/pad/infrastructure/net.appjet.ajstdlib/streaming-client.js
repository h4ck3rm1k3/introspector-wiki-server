...
function WebSocket(id) {
  ...
  this.onhiccup = function() {
      ...
      console.log("onhiccup");
}
  this.onlogmessage = function(x) {
      console.log(x);
  }
  this.CONNECTING = 0;
  this.OPEN = 1;
  ...
  function log(s) {
  self.onlogmessage("(comet @t: "+nicetime()+") "+s);
  }
  function logAll() {
    log(self.describe())
  }
  ...
  function simpleXhr(method, uri, async, params, cb, makeXhr) {
    ...
    if (async) {
      request.onreadystatechange = function() {
	    ...
	    console.log(e);
	/* absorb ff error accessing request properties */ }
    ...
    } catch (e)
  {
    console.log(e);
    request.abort();
      cb(500, "Error sending data!", request);
      }
    if (! async) {
      var status;
      var responseText;
    ...
    function postFailed(sc, msg, req, src) {
      ...
      try {
        str = sc + ": "+req.statusText+" - "+msg+" ("+src+")";
          ...
          console.log(e);
	/* absorb potential Firefox error accessing req */
      }
  ...
  function postPendingMessages() {
    ...
    isPosting = true;
    doPostMessages(messages, false, false,
    function() {
      log("do post pending messages");
      isPosting = false;
	setTimeout(postPendingMessages, 0);
    });
    lastPost = nicetime();
  }
  ...
  /*
    the magic function that needs to be understood! this is the key to polling.
here is one response:
length:something:text:
13:oob:keepalive
  ...
  function ShortPollingChannel() {
    ...
    /*
todo : this is the function that manages the polling data!
     */
  ...
  function StreamingChannel() {
	...
	return;
      ...
      timeout(timeouts, "data", 60*1000, function() {
               log("Timeout ");
		hiccup(self);
    ...
    this.connect = function() {
	  ...
	  + host
	    +channelPath()
	      +"&channel=streaming&type=iframe&new=yes&create="
		+(socket.readyState == socket.OPEN ? "no" : "yes")
		  +"&seq="
		    +lastReceivedSeqNumber
		      + "&random="
			+ randomVar();
...
//          theStream.attr('src', streamurl);
  ...
  function ExtHostXHR(iframe) {
    ...
    this.send = function(data) {
      ...
      if (iframe)
	{
	  this.xhr = iframe.iframe.contentWindow.doAction(
		       this.method, this.uri, this.async, headers, data || null,
	...
	{
	  console.log ("no iframe object defined!");
	}
  ...
  function createRequestIframe(cb) {
    ...
    window["done_"+randomKey] = function() {
      log("done with window");
      try { delete window["done_"+randomKey]; }
				  catch (e) {
				      console.log(e);
				  }; cb(); }
